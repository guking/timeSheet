...
def over_time(end_time, start_time):
    ...
    :return: 加班时长
    """
    result = (end_time - pd.to_datetime(str(start_time)[0:10] + " 18:00:00")) / pd.Timedelta(1, 'min') / 60
    return result
...
def cal_diff(start_time, end_time):
    ...
    if is_holiday(start_time):
        if '02:00:00' <= str(start_time)[11:]:
            ...
            overtime = num_cal((end_time - start_time) / pd.Timedelta(1, 'min') / 60)
    ...
    # 工作日考勤
        ...
        if str(start_time)[11:] < '09:15:00':
            if '17:30:00' <= str(end_time)[11:] < '20:00:00':
                delta = num_cal((end_time - start_time) / pd.Timedelta(1, 'min') / 60)
...
def emp_st1(_file_url):
    ...
    """
    output_prefix = prefix(_file_url)
    emp_file = output_prefix[:output_prefix.rfind('\\')] + '\\考勤名单.xlsx'
    df_st1 = pd.read_excel(emp_file, sheet_name='Sheet1', header=0).reset_index()
    df_st1['value'] = 0
    df_st1.columns = ['id', '职工姓名', '考勤编号', '角色', '办公场地', 'value']
    # print('df_st1\n', df_st1)
    return df_st1


def time_sheet(_sheet_context, list_emp):
    """
...
if __name__ == '__main__':
    ...
    print('输入的考勤记录文件路径为: ', args)
    # args = "D:\恒格文档\考勤\7月金科职场外包考勤.xls"
    file_url = args
    ...
    sheet_context = pd.read_excel(file_url, sheet_name='Sheet4', header=0)
    excel_file_name = '.xlsx'

    # sheet1的考勤人员名单
    df_emp_st1 = emp_st1(file_url)
    # print('df_emp_st1\n', df_emp_st1)
    list_emp_st1 = df_emp_st1['职工姓名'].to_list()
    # sheet2的考勤人员名单
    df_emp_st2 = emp_st2(file_url)
    list_emp_st2 = df_emp_st2['职工姓名'].to_list()

    # 进入日期 和 进入时间字段合并
    sheet_context['attend_datetime'] = pd.to_datetime(sheet_context['进入日期'].astype(str).str[:10]
                                                      + " " + sheet_context['进入时间'].map(str))
    # 过滤出sheet2人员名单的timesheet考勤记录
    time_sheet_st1 = time_sheet(sheet_context, list_emp_st1)
    time_sheet_st2 = time_sheet(sheet_context, list_emp_st2)
    # print('time_sheet_st1\n', time_sheet_st1)
    # out_date = '_' + pd.datetime.now().strftime('%Y%m%d%H%M%S') + '生成st1'
    # writer0 = pd.ExcelWriter(prefix(file_url) + out_date + excel_file_name)
    # time_sheet_st1.to_excel(writer0)
    # writer0.save()
    # writer0.close()
    ...
    df1['异常工时'] = df1['异常工时'] / 8
    attend1 = df1[['职工姓名', '考勤编号', '角色', '办公场地', '日期', '开始时间', '实际结束时间',
                   '正常工时', '加班工时', '异常工时', '是否节假日']].fillna(0)
